...
// Adapted by Willy McAllister for Khan Academy.
...
schematic = (function() {
    ...
    var component_style = 'rgb(60,145,229)';  	// color for unselected components WMc KA default5, was 64,64,255 
	...
	function Schematic(input) {
		...
		//this.tools['clr'] = this.add_tool('CLEAR','Clear Net List',this.clear_netlist);			//WMc  Clear is not needed
	    //this.enable_tool('clr',true);	

	    this.tools['net'] = this.add_tool('DOWNLOAD','DOWNLOAD Net List',this.download_netlist);		//WMc
	    ...
	    this.zctl_left = this.sctl_x - 8;	//zoom control parameters
		...
		this.rctl_y = this.zctl_top + this.zctl_h + 8 + this.rctl_r;
		...
		this.canvas.addEventListener('wheel',schematic_mouse_wheel,false);		//WMc mousewheel deprecated. Use onwheel.
...
/*
	Schematic.prototype.clear_netlist = function() {		//WMc new function  - Clear function not needed
...
*/
	...
	Schematic.prototype.setup_ac_analysis = function() {
		...
		alert("AC Analysis: add a voltage probes to the diagram!");	//WMc there are no voltage probes in the diagram!
		return;
	    }
	...
	Schematic.prototype.transient_analysis = function() {
	    ...
	    var probes = this.find_probes();
	    if (probes.length == 0) {
		alert("Transient Analysis: add a probe to the diagram!");	//WMc there are no probes in the diagram!
		return;
	    }
	...
	function schematic_mouse_down(event) {
	    ...
	    } else if (zx >= 0 && zx < 16 && zy >= 0 && zy < 48) {   // clicked in zoom control  QQQ this.zctl_w = 16 and this.zctl_h = 48
		if (zy < 16) sch.zoomin();
		else if (zy < 32) sch.zoomout();
		else sch.zoomall();
	    } 
	    else if (rx*rx + ry*ry <= sch.rctl_r*sch.rctl_r) {   // clicked in rotation control
	    	console.log("clicked in rotation control");
	...
	Schematic.prototype.window = function(title,content,offset) {
	    ...
	    head.style.backgroundColor = 'rgb(19,78,163)';				//WMc 'black' KAdefault5 = 19,78,163
