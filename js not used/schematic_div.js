...
schematic = (function() {
	...
	if (this.edits_allowed) {
		...
		this.tools['spacer'] = this.add_tool(spacer_icon,'',this.help);
	...
	if (this.edits_allowed) {
	    ...
	    if (false){
	    	...
	    	if (!this.diagram_only) {
	    		...
	    		outerdiv.style.borderColor = normal_style;
	    		outerdiv.style.backgroundColor = background_style;
	    	}
    	    // add tools to DOM
    	    if (this.toolbar.length > 0) {
    	    	...
    	    	tooldiv.style.borderStyle = 'solid';
    	    	tooldiv.style.borderWidth = '2px';
    	    	tooldiv.style.borderColor = normal_style;
    	    	for (var i = 0; i < this.toolbar.length; ++i) {
    	    		var tool = this.toolbar[i];
    	    		if (tool != null) tooldiv.appendChild(tool);
    	    	}
    	    }

	    	/*
	    	<div style="border: 1px solid blue; margin: auto; width: 400;">
	    		...
	    		<div style="border: 1px solid green; clear: both;">
	    			status div
	    		</div>
	    	</div>
	    	*/
...
var spacer_icon = 'data:image/png;base64,'
	...
	function engineering_notation(n,nplaces,trim) {
		if (n == 0) return '0';
		if (Math.abs(n) < 1e-20) return '0';	//WMc flatten tiny numbers to zero
...
if (z_values != undefined && z_values.length > 0) {
	...
	function plot_cursor(c,graph,cursor_x,left_margin) {
	    ...
	    c.textBaseline = 'top';
	    c.fillStyle = background_style;
	    c.globalAlpha = 0.8;	//WMc
	    c.fillText('\u2588\u2588\u2588\u2588\u2588',x,end_y);
	    c.globalAlpha = 1.0;	//WMc
	    c.fillStyle = normal_style;
