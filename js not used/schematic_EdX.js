...
//The window.onload line was removed by EdX (SJSU), with this warning
/*
...
schematic = (function() {
    var background_style 	= 'rgb(240,241,242)';	//WMc KA gray95, was 220,220,220
    var element_style = 'rgb(255,255,255)';
    var thumb_style = 'rgb(128,128,128)';
    var normal_style = 'rgb(0,0,0)';  			// default drawing color
    var component_style = 'rgb(60,145,229)';  	// color for unselected components WMc KA default5
    var selected_style = 'rgb(116,207,112)';	// highlight color for selected components WMc KA CS2
	...
	function Schematic(input) {
	    ...
	    this.tools['net'] = this.add_tool('DOWNLOAD','DOWNLOAD Net List',this.download_netlist);
	    this.enable_tool('net',true);   
	    
	    this.tools['load'] = this.add_tool('IMPORT','Import Net List',this.import_netlist);
	...
	Schematic.prototype.help = function() {
		...
		var strSel  = "Select: Click or drag a rectangle to select components. \nShift-click to include another component.\n\n";
		var strMove = "Move: Click to select, drag to new location.\n\n";
		var strDel  = "Delete: Click to select, then DELETE or BACKSPACE.\n\n";
		var strRot  = "Rotate/Reflect: Select a component, then type the letter \"r\" to rotate 90. Type \“r\” again for more rotations and reflections.\n\n";
		var strProp = "Properties: Double click on a component to bring up a window to change values.\n\n";
	...
	Schematic.prototype.download_netlist = function() { 	//WMc new function
	...
	Schematic.prototype.import_netlist = function() {		//WMc new function
	    ...
	    this.dialog('Import Netlist',content,function(content) {
		    var sch = content.sch;
	...
	Schematic.prototype.setup_ac_analysis = function() {
	    ...
	    var npts_lbl = 'Number of points/decade';					//WMc not used(?)
	...
	function schematic_mouse_down(sch) {
	    ...
	    else if (rx*rx + ry*ry <= sch.rctl_r*sch.rctl_r) {   // clicked in rotation control (same as "r" key)
	    	for (var i = sch.components.length - 1; i >= 0; --i) {
	    	    var component = sch.components[i];
	    	    if (component.selected) {
	    		component.rotate(1);
	    ...
	    else if (dx*dx + dy*dy <= sch.rctl_r*sch.rctl_r) {   // clicked in delete control (same as delete key)
			// delete selected components
			for (var i = sch.components.length - 1; i >= 0; --i) {
		    	var component = sch.components[i];
		    	if (component.selected) component.remove();
	...
	function schematic_double_click(event) {
	    ...
	    //var sch = (window.event) ? event.srcElement.schematic : event.target.schematic;
	    var sch = event.target.schematic; 	//WMc needed for Hammer.js
	...
	Schematic.prototype.window = function(title,content,offset) {
	    ...
	    head.style.backgroundColor = 'rgb(202,51,124)';		//WMc KA Science1 or gray68: rgb(136,141,147)
	...
	Part.prototype.set_component = function(component,tip) {
	    ...
	    var dx = b[2] - b[0];
	    var dy = b[3] - b[1];
	    this.scale = 1.0; //Math.min(part_w/(1.2*dx),part_h/(1.2*dy));	//WMc 0.8
	...
	Part.prototype.redraw = function(part) {
	    ...
	    // paint background color     WMc commmented out for mobile, (stays on)
	...
	function part_mouse_up(event) {
	    ...
	    //part.sch.new_part = undefined;		// place parts with touch-touch on mobil
    ...
    var probe_colors_rgb = {
	    'red': 'rgb(232,77,57)',			// WMc rgb(255,64,64)',
	    'green': 'rgb(31,171,84)',			// WMc KA CS1, was rgb(64,255,64)',
	    'blue': 'rgb(35,110,201)',			// WMc rgb(64,64,255)',
	    'cyan': 'rgb(99,217,234)',			// WMc rgb(64,255,255)',
	    'magenta': 'rgb(237,95,166)',		// WMc rgb(255,64,255)',
	    'yellow': 'rgb(244,211,69)',		// WMc rgb(255,255,64)',
	    'black': 'rgb(0,0,0)',
	    'x-axis': undefined
	};
