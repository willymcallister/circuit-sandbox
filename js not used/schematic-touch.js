...
var cktsim = (function() {
	...
	Circuit.prototype.load_netlist = function(netlist) {
	    ...
	    if (!found_ground) { // No ground on schematic
	    	alert('Please make at least one connection to ground');
	    	return false;
	    }
	    return true;
	}
...
function update_schematics() {
    ...
    var schematics = document.getElementsByClassName('schematic');	//WMc restored line from MIT version
    ...
    for (var i = 0; i < schematics.length; ++i)
    	if (schematics[i].getAttribute("loaded") != "true") {
    		try {
    			...
    			var msg = document.createTextNode('Sorry dude, there was a browser error in starting the schematic tool.  We recommend the latest versions of Firefox and Chrome.');
...
//This line was removed by EdX (SJSU), with this warning
/*
...
*/
window.onload = add_schematic_handler(window.onload);	//WMc restored
...
schematic = (function() {
    var background_style 	= 'rgb(227,229,230)';	//WMc 220,220,220 KA gray90
    var element_style = 'rgb(255,255,255)';
    var thumb_style = 'rgb(128,128,128)';
    var normal_style = 'rgb(0,0,0)';  // default drawing color
    var component_style = 'rgb(60,145,229)';  	// color for unselected components WMc 64,64,255 KA default5
    var selected_style = 'rgb(116,207,112)';  	// highlight color for selected components WMc 64,255,64 KA CS2
    var grid_style = "rgb(128,128,128)";
    var annotation_style = 'rgb(255,64,64)';  // color for diagram annotations
    var property_size = 7;  // point size for Component property text 	WMc 5
    var annotation_size = 7;  // point size for diagram annotations 	WMc 6
	...
	function Schematic(input) {
	    ...
	    if (!this.diagram_only) {
	    	this.tools['help'] = this.add_tool(help_icon,'Touch: Help: display help page',this.help);
	...
	if (this.edits_allowed) {
		...
		this.tools['clr'] = this.add_tool('CLEAR','Clear Net List',this.clear_netlist);			//WMc
		this.enable_tool('clr',true);	

	    this.tools['net'] = this.add_tool('EXPORT','Export Net List',this.export_netlist);		//WMc
	    this.enable_tool('net',true);															//WMc	    
	    
	    this.tools['load'] = this.add_tool('IMPORT','Import Net List',this.import_netlist);		//WMc
	    this.enable_tool('load',true);															//WMc
	    this.toolbar.push(null);  // spacer	
	}
	    ...
	    this.canvas.schematic = this;		//QQQ please explain - looks reentrant
	    if (this.edits_allowed) {
			...
			//console.log("double tap! x: " + event.center.x + "  y: " + event.center.y);
			var sch = event.target.schematic;		
		...
		//this.canvas.addEventListener('dblclick',schematic_double_click,false);	//replaced by Hammer doubletap
		...
		this.status_div.style.font = '12pt sans-serif';	//WMc
...
Schematic.prototype.unselect_all = function(which) {
	...
	//    window.open('/static/handouts/schematic_tutorial.pdf');
		var strSHelp = "SCHEMATIC HELP\n\n";		//WMc embedded Help 
		var strAddC = "Add component: Drag from the parts bin onto the diagram.\n\n";
		...
		var strSel  = "Select: Click to select component. \nShift-click to add a component to the current selection. \nDrag a rectangle to select intersecting components.\n\n";
		var strMove = "Move: Click to select, drag to new location.\n\n";
		var strDel  = "Delete: Click to select, then DEL or BACKSPACE.\n\n";
		var strRot  = "Rotate/Reflect: Select component, then type the letter \"r\" to rotate 90 degrees. Type \“r\” repeatedly for additional rotations and reflections.\n\n";
		var strProp = "Change properties: Double click on component to bring up an Edit Properties window.\n\n";
...
Schematic.prototype.toggle_grid = function() {
	...
	var zoom_wheel_factor = 1.05;    //WMc clam zooming with wheel, since wheel events arrive in quick succession
	var zoom_min = 0.5;
	var zoom_max = 4.0;
...
this.redraw_background();
	...
	Schematic.prototype.clear_netlist = function() {		//WMc new function
		var clrNetlist = window.prompt("Clearing the netlist. Are you sure?", "yes or no");
		if (clrNetlist == "yes") {
	    	this.components = [];								//WMc clear components
	    	this.connection_points = [];						//WMc clear connection points
	    	this.redraw();
	    	this.zoomall();
	    }
	}

	Schematic.prototype.export_netlist = function() {		//WMc new function
	    // give all the circuit nodes a name, export netlist
	    this.label_connection_points();
	    var netlist = this.json();
	    this.input.value = JSON.stringify(netlist);

	    window.confirm('JSON netlist: \nCopy and paste the following to a text file\n\n' + this.input.value + '\n');
	}

	Schematic.prototype.import_netlist = function() {		//WMc new function
		var myNetlist = window.prompt("Paste JSON Netlist", "paste JSON netlist here");
	    if (myNetlist[0] == "[") {								//WMc FIX: this needs a 'try/catch' if parsing fails
	    	//var myJSON = JSON.parse(myNetlist);
	    console.log("Netlist starts with a bracket [.");
	    	this.components = [];								//WMc clear components
	    	this.connection_points = [];						//WMc clear connection points
	    	this.load_schematic(myNetlist);	    				// process new contents of diagram
	    	this.zoomall();
	    }
	}
...
Schematic.prototype.transient_analysis = function() {
	...
	try {
		if (HTMLCanvasElement)
		    ...
		    var currentElement = this; // whatever element .relMouseCoords was called on
...
function schematic_mouse_wheel(event) {
	if (!event) event = window.event;
	    ...
	    else if (event.detail) delta = -event.detail;		//WMc what does this do?

	    if (delta) {
		var nscale = (delta > 0) ? sch.scale*zoom_wheel_factor : sch.scale/zoom_wheel_factor;	//WMc
...
Schematic.prototype.dialog = function(title,content,callback) {
	    ...
	    body.style.font = '10pt sans-serif';							//WMc new font spec
	    ...
	    ok_button.style.font = '10pt sans-serif';						//WMc new font spec
	    */

		var ok_button = document.createElement('span');					//WMc checkmark icon instead of "OK" text
	    //ok_button.appendChild(document.createTextNode('\u2713'));
	    var ok_icon = document.createElement("img");
	    ok_icon.src = 'data:image/png;base64,'
	    ...
	    ok_button.addEventListener('click',dialog_okay,false);
	    ok_button.style.display = 'inline';
	    ok_button.style.border = '0px solid';							//WMc 1px
	    ok_button.style.padding = '5px';
	    ok_button.style.margin = '10px';
	    ok_button.style.font = '10pt sans-serif';						//WMc new font spec

	    var cancel_button = document.createElement('span');				//WMc X icon instead of "CANCEL" text
	    //cancel_button.appendChild(document.createTextNode('CANCEL'));	//WMc 'Cancel'
	    ...
	    cancel_button.style.border = '0px solid';						//WMc 1px
	    cancel_button.style.padding = '5px';
	    cancel_button.style.margin = '10px';
	    cancel_button.style.font = '10pt sans-serif';					//WMc new font spec
...
function build_table(a) {
	...
	Schematic.prototype.window = function(title,content,offset) {
	    ...
	    head.style.backgroundColor = 'rgb(202,51,124)';			//WMc 'black' Science1
	    head.style.font = '12pt sans-serif';					//WMc new font spec
	...
	//var close_icon = 'data:image/png;base64,'
...
function redraw_plot(graph) {
	...
	function Part(sch) {
	    ...
	    this.canvas.addEventListener('click',function(){},false);		//QQQ verify this doesn't interfere with tap handler
	}
	...
	Part.prototype.redraw = function(part) {
		var c = this.canvas.getContext('2d');
	    //WMc taken out, black border sufficient to indicate selection
	    // paint background color
...
function part_mouse_down(event) {
	...
	function part_mouse_up(event) {
	    ...
	    //part.select(false);					//WMc (I forget why.)
	    //part.sch.new_part = undefined;		//WMc
	    return false;
	}
...
ConnectionPoint.prototype.update_location = function() {
	...
	Ground.prototype.draw = function(c) {
	    ...
	    this.draw_line(c,-6,8,0,14);	//WMc triangle
	    this.draw_line(c,0,14,6,8);		//WMc
	}
...
Ammeter.prototype.draw = function(c) {
	...
	Resistor.prototype.draw = function(c) {
		...
		this.draw_text(c,this.properties['r']+'\u03A9',8,24,3,property_size);	//WMc 5,24,3; 8,24,3
	if (this.properties['name'])
		this.draw_text(c,this.properties['name'],-8,24,5,property_size);		//WMc -5,24,5; -8,24,5
}
...
Resistor.prototype.clone = function(x,y) {
	...
	Inductor.prototype.draw = function(c) {
		...
		this.draw_text(c,this.properties['l']+'H',8,24,3,property_size);	//WMc 6,24,3
	if (this.properties['name'])
		this.draw_text(c,this.properties['name'],-6,24,5,property_size);	//WMc -3,24,5
}
...
Inductor.prototype.clone = function(x,y) {
	...
	Diode.prototype.draw = function(c) {
	    Component.prototype.draw.call(this,c);   // give superclass a shot
	    this.draw_line(c, 0,0,0,18);		//WMc 0,0,0,16
	    this.draw_line(c,-8,18,8,18);		//WMc -8,16,8,16
	    this.draw_line(c,-8,18,0,30);		//WMc -8,16,0,32
	    this.draw_line(c, 8,18,0,30);		//WMc 8,16,0,32
	    this.draw_line(c,-8,30,8,30);		//WMc -8,32,8,32
	    this.draw_line(c, 0,30,0,48);		//WMc 0,32,0,48
...
Diode.prototype.edit_properties = function(x,y) {
	...
	function OpAmp(x,y,rotation,name,A) {
	    ...
	    this.add_connection(24,16);  // ground    WMc 24,32
	    this.bounding_box = [0,-8,48,24];		//WMc 0,-8,48,32
	    this.update_coords();
	}
	...
	OpAmp.prototype.draw = function(c) {
	    ...
	    this.draw_line(c,8,-8,32,8);		//WMc 8,-8,40,8 top slant
	    this.draw_line(c,8,24,32,8);		//WMc 8,24,40,8 bottom slant
	    // inputs and output
	    this.draw_line(c,0,0,8,0);			//WMc 0,0,8,0   + input port
	    this.draw_line(c,0,16,8,16);		//WMc 0,16,8,16 - input port
	    this.draw_text(c,'g',30,20,property_size);	//WMc 'gnd',37,18
	    this.draw_line(c,32,8,48,8);		//WMc 40,8,48,8 output port
	    this.draw_line(c,24,13,24,16);		//WMc 24,16,24,32 gnd port
	    // + and -
	    this.draw_line(c,10,0,14,0);		//WMc 10,0,16,0  + horizontal stroke
	    this.draw_line(c,12,-2,12,2);		//WMc 13,-3,13,3 + vertical stroke
	    this.draw_line(c,10,16,14,16);		//WMc 10,16,16,16 - stroke

	    if (this.properties['name'])
		this.draw_text(c,this.properties['name'],24,-8,0,property_size);	//WMc 32,16,0
}
...
OpAmp.prototype.clone = function(x,y) {
	...
	if (this.properties['name'])
		this.draw_text(c,this.properties['name'],-16,24,5,property_size);	//WMc -13,24,5
	if (this.properties['value'])
		this.draw_text(c,this.properties['value'],16,24,3,property_size);	//WMc  13,24,3
}
